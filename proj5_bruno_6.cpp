...
enum TestPhase {
    INITIAL_RESET_START, INITIAL_RESET_END,
    WAIT_FOR_TEST1, RUN_TEST1, SAMPLE_TEST1,
    WAIT_FOR_TEST2, RUN_TEST2, SAMPLE_TEST2,
    WAIT_FOR_TEST3, RUN_TEST3, SAMPLE_TEST3,
    FINAL_RESET_START, FINAL_RESET_END,
    SIM_DONE
};
...
int main(int argc, char** argv) {
    ...
    dut->reset = 1; // Start with reset de-asserted (active-low)
    dut->INI = 0;
    dut->A = 0;
    dut->B = 0;

    while (sim_time < 8000 && !Verilated::gotFinish()) {
        
        dut->clk = 0; dut->eval(); m_trace->dump(sim_time); sim_time += 5;

        switch(test_phase) {
            case INITIAL_RESET_START:
                if (sim_time >= 10) {
                    std::cout << "[" << sim_time << " ns] Applying initial reset pulse..." << std::endl;
                    dut->reset = 0; // Assert reset (active-low)
                    dut->eval();
            ...
            case RUN_TEST1:
                dut->INI = 0;
                if (dut->P == 1) test_phase = SAMPLE_TEST1;
                break;
            case SAMPLE_TEST1:
            ...
            case RUN_TEST2:
                dut->INI = 0;
                if (dut->P == 1) test_phase = SAMPLE_TEST2;
                break;
            case SAMPLE_TEST2:
                std::cout << "[" << sim_time << " ns] Division 2 complete (P=1 detected)." << std::endl;
                std::cout << "  Result: R=" << (int)dut->R << ", REM=" << (int)dut->REM << std::endl;
                test_phase = WAIT_FOR_TEST3;
                break;
            case WAIT_FOR_TEST3:
                if (sim_time >= 500) {
                    dut->A = 5; dut->B = 8;
                    std::cout << "[" << sim_time << " ns] Test 3 (A < B): A=" << (int)dut->A << ", B=" << (int)dut->B << std::endl;
                    dut->INI = 1;
                    test_phase = RUN_TEST3;
                }
                break;
            case RUN_TEST3:
                dut->INI = 0;
                if (dut->P == 1) test_phase = SAMPLE_TEST3;
                break;
            case SAMPLE_TEST3:
                std::cout << "[" << sim_time << " ns] Division 3 complete (P=1 detected)." << std::endl;
                std::cout << "  Result: R=" << (int)dut->R << ", REM=" << (int)dut->REM << ". Expected: R=0, REM=5" << std::endl;
                test_phase = FINAL_RESET_START;
                break;
            case FINAL_RESET_START:
                std::cout << "[" << sim_time << " ns] All tests finished. Applying final reset..." << std::endl;
                dut->reset = 0; // Assert final reset (active-low)
                dut->eval();
            ...
            case FINAL_RESET_END:
                if (sim_time >= reset_end_time) {
                    dut->reset = 1; // De-assert final reset
                    dut->eval();
        ...
        dut->clk = 1; dut->eval(); m_trace->dump(sim_time); sim_time += 5;
    }
